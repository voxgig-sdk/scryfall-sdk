...
boru-driven command-line client **and** interactive REPL for the Scryfall
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/scryfall-cli)
make build
...
./scryfall-cli list bulk_data
./scryfall-cli load 1 bulk_data            # {id:1} shorthand
./scryfall-cli load '{id:1}' bulk_data       # explicit match map
./scryfall-cli list card

# 5. Override the API base URL for a single call
SCRYFALL_BASE=https://api.example.com ./scryfall-cli list bulk_data

# 6. No arguments -> interactive REPL
./scryfall-cli
scryfall> list bulk_data
scryfall> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/scryfall-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/scryfall-cli list bulk_data
   ```
...
```sh
./scryfall-cli load 1 bulk_data          # scalar shorthand for {id:1}
./scryfall-cli load '{id:1}' bulk_data     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `scryfall>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bulk_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/scryfall-cli`. |
...
(`./scryfall-cli <expr>`) and interactively (the REPL), and expressions compose the
