...
local RulingEntity = {}
RulingEntity.__index = RulingEntity


function RulingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RulingEntity)
  self._name = "ruling"
  self._client = client
...
function RulingEntity:get_name()
  return self._name
end


function RulingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RulingEntity.new(self._client, opts)
end
...
function RulingEntity:mark_deleted()
  self._deleted = true
end


function RulingEntity:deleted()
  return true == self._deleted
end


function RulingEntity:data_set(args)
  if args ~= nil then
...
function RulingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RulingEntity:match_set(args)
  if args ~= nil then
...
function RulingEntity:match_get()
...
function RulingEntity:stream(action, args, callopts)
...
function RulingEntity:_run_op(ctx, post_done)
  local utility = self._utility
