...
local CardListEntity = {}
CardListEntity.__index = CardListEntity


function CardListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardListEntity)
  self._name = "card_list"
  self._client = client
...
function CardListEntity:get_name()
  return self._name
end


function CardListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardListEntity.new(self._client, opts)
end
...
function CardListEntity:mark_deleted()
  self._deleted = true
end


function CardListEntity:deleted()
  return true == self._deleted
end


function CardListEntity:data_set(args)
  if args ~= nil then
...
function CardListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardListEntity:match_set(args)
  if args ~= nil then
...
function CardListEntity:match_get()
...
function CardListEntity:stream(action, args, callopts)
...
---@param reqmatch CardListListMatch
---@param ctrl? table
---@return CardList[]
---@return string? err
function CardListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CardListCreateData
---@param ctrl? table
---@return CardList
---@return string? err
function CardListEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CardListEntity:_run_op(ctx, post_done)
  local utility = self._utility
