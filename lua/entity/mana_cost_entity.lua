...
local ManaCostEntity = {}
ManaCostEntity.__index = ManaCostEntity


function ManaCostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ManaCostEntity)
  self._name = "mana_cost"
  self._client = client
...
function ManaCostEntity:get_name()
  return self._name
end


function ManaCostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ManaCostEntity.new(self._client, opts)
end
...
function ManaCostEntity:mark_deleted()
  self._deleted = true
end


function ManaCostEntity:deleted()
  return true == self._deleted
end


function ManaCostEntity:data_set(args)
  if args ~= nil then
...
function ManaCostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ManaCostEntity:match_set(args)
  if args ~= nil then
...
function ManaCostEntity:match_get()
...
function ManaCostEntity:stream(action, args, callopts)
...
function ManaCostEntity:_run_op(ctx, post_done)
  local utility = self._utility
