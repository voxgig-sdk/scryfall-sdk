...
describe("BulkDataDirect", function()
  it("should direct-list-bulk_data", function()
    local setup = bulk_data_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bulk_data", setup.live and "live" or "unit")
  ...
  it("should direct-load-bulk_data", function()
    local setup = bulk_data_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-bulk_data", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function bulk_data_direct_setup(mockres)
