...
The Ruby SDK for the Scryfall API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.BulkData(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Scryfall(nil).load(
  { "id" => "test01" }, nil
)
