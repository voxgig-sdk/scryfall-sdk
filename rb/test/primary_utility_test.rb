...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ScryfallOmni.make_runner(TEST_JSON_FILE, ScryfallSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ScryfallOmni.errify(err)
    end
