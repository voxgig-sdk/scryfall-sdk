...
class CardSymbolListDirectTest < Minitest::Test
  def test_direct_list_card_symbol_list
    setup = card_symbol_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-card_symbol_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def card_symbol_list_direct_setup(mockres)
