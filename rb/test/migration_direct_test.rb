...
class MigrationDirectTest < Minitest::Test
  def test_direct_list_migration
    setup = migration_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-migration", setup[:live] ? "live" : "unit")
    if _should_skip
...
def migration_direct_setup(mockres)
