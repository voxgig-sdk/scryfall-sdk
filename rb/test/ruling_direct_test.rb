...
class RulingDirectTest < Minitest::Test
  def test_direct_list_ruling
    setup = ruling_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-ruling", setup[:live] ? "live" : "unit")
    if _should_skip
...
def ruling_direct_setup(mockres)
