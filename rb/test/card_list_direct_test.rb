...
class CardListDirectTest < Minitest::Test
  def test_direct_list_card_list
    setup = card_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-card_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def card_list_direct_setup(mockres)
