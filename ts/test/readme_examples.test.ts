...
//     (\`new ScryfallSDK(...)\` / \`ScryfallSDK.test(...)\` become
//     \`ScryfallSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new ScryfallSDK(...)\` and any
// \`ScryfallSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ScryfallSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ScryfallSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
