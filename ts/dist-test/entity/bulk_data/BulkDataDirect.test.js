...
(0, node_test_1.describe)('BulkDataDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-bulk_data', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-bulk_data', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-bulk_data', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-bulk_data', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.SCRYFALL_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ScryfallSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['SCRYFALL_TEST_BULK_DATA_ENTID'];
    ...
    const client = new __1.ScryfallSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BulkDataDirect.test.js.map