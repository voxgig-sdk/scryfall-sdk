...
class ScryfallSDK
{
        ...
        if ($feature_opts) {
            ...
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ScryfallHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ScryfallFeatures::has_feature($fname)) {
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ScryfallError($op . "_allow",
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ScryfallError("graphql_error",
                "ScryfallSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
