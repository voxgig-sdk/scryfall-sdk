...
class FtCtrl extends ScryfallControl
{
...
class FtClient
{
    ...
    private ScryfallUtility $utility;

    public function __construct(ScryfallUtility $utility, array $options)
    {
    ...
    public function get_utility(): ScryfallUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ScryfallContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ScryfallContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ScryfallContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ScryfallConfig::make_feature($name);
    ...
    private static function buildUrl(ScryfallSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ScryfallError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ScryfallResponse($fetched);
        }
    ...
    private function fail(ScryfallContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ScryfallContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
