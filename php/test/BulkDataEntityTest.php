...
class BulkDataEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ScryfallSDK::test(null, null);
        $ent = $testsdk->BulkData(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->BulkData(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->BulkData(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = bulk_data_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "bulk_data." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SCRYFALL_TEST_BULK_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $bulk_data_ref01_data = null;
        if (count($bulk_data_ref01_data_raw) > 0) {
...
function bulk_data_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/bulk_data/BulkDataTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["bulk_data01", "bulk_data02", "bulk_data03"] as $k) {
    ...
    $entid_env_raw = getenv("SCRYFALL_TEST_BULK_DATA_ENTID");
