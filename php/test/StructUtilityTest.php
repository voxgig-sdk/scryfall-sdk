...
// Scryfall SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ScryfallOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ScryfallOmni::structfix($now, Struct::undef()))
                        !== json_encode(ScryfallOmni::structfix($val, Struct::undef()))) {
