...
class BulkDataEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BulkDataEntity($this->_client, $opts);
    }

    /**
     * @param BulkData|array $args BulkData data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ScryfallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return BulkData|array The current BulkData data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of BulkData fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ScryfallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of BulkData fields).
     */
    ...
    /**
     * Load a single BulkData.
     *
     * @param BulkDataLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BulkDataLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return BulkData|array The loaded BulkData as an assoc-array at the
     *   SDK boundary; throws ScryfallError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ScryfallHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List BulkData items matching the given filter.
     *
     * @param BulkDataListMatch|array|null $reqmatch Match filter (any subset
     *   of BulkData fields) as an assoc-array; BulkDataListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return BulkData[]|array A list of BulkData items as assoc-arrays at
     *   the SDK boundary; throws ScryfallError on failure (item-5 convention).
     */
