...
class ScryfallRetryFeature extends ScryfallBaseFeature
{
    ...
    public function init(ScryfallContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ScryfallContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ScryfallContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ScryfallContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
