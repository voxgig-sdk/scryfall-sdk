...
{
    private static function utility(): ScryfallUtility
    {
        return new ScryfallUtility();
    }
    ...
    private static function ctx(array $over = []): ScryfallContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ScryfallContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ScryfallOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ScryfallError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ScryfallOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ScryfallMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ScryfallOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ScryfallMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ScryfallOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ScryfallMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ScryfallSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ScryfallMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ScryfallResponse([]),
            'result' => new ScryfallResult([]),
        ]);
        [, $err] = ScryfallMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ScryfallSpec([]),
            'response' => null,
            'result' => new ScryfallResult([]),
        ]);
        [, $err] = ScryfallMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ScryfallSpec(['step' => 's']),
            'response' => new ScryfallResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ScryfallResult([]),
        ]);
        [, $err] = ScryfallMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ScryfallSpec(['step' => 's']),
            'response' => new ScryfallResponse(self::resp(200, ['v' => 1])),
            'result' => new ScryfallResult([]),
        ]);
        [, $err] = ScryfallMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ScryfallSpec(['step' => 's']),
            'response' => new ScryfallResponse(self::resp(200, ['v' => 2])),
            'result' => new ScryfallResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ScryfallMakeResponse::call($ctx);
    ...
    {
        $preset = new ScryfallResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ScryfallSpec([]),
            'response' => new ScryfallResponse([]),
            'result' => new ScryfallResult([]),
        ...
        [$got, $err] = ScryfallMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ScryfallResult([])]);
        [, $err] = ScryfallMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ScryfallSpec([]), 'result' => null]);
        [, $err] = ScryfallMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ScryfallSpec(['step' => 's']),
            'result' => new ScryfallResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ScryfallOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ScryfallMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ScryfallSpec(['step' => 's']),
            'result' => new ScryfallResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ScryfallOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ScryfallMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ScryfallResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ScryfallSpec([]),
            'result' => new ScryfallResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ScryfallMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ScryfallMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ScryfallError('boom', 'boom');
        $utility->fetcher = function (ScryfallContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ScryfallSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ScryfallMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ScryfallContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ScryfallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ScryfallResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ScryfallContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ScryfallSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ScryfallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ScryfallResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ScryfallSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ScryfallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ScryfallMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ScryfallMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ScryfallResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ScryfallDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ScryfallResult(['ok' => false])]);
        $this->expectException(ScryfallError::class);
        ScryfallDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ScryfallResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ScryfallMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ScryfallMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ScryfallBaseFeature();
        $b = new ScryfallBaseFeature();
        ScryfallFeatureAdd::call($ctx, $a);
        ScryfallFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ScryfallBaseFeature
    {
        $f = new ScryfallBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ScryfallFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ScryfallContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ScryfallSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ScryfallPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ScryfallResponse(['headers' => null]),
            'result' => new ScryfallResult([]),
        ]);
        ScryfallResultHeaders::call($ctx);
