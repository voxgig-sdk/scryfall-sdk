...
module ScryfallUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ScryfallResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ScryfallResponse.new(fetched)
    else
