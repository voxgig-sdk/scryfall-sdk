...
type Args struct {
	Entity string         `json:"entity" jsonschema:"bulk_data | card | card_list | card_symbol_list | catalog | mana_cost | migration | ruling | set"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ScryfallSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "scryfall_list",
		Description: "List records from Scryfall. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "scryfall_load",
		Description: "Load a single record from Scryfall. " +
...
func runOp(client *sdk.ScryfallSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ScryfallSDK, name string) (sdk.ScryfallEntity, error) {
	switch strings.ToLower(name) {
	case "bulk_data":
		return client.BulkData(nil), nil
	case "card":
	...
	case "mana_cost":
		return client.ManaCost(nil), nil
	case "migration":
		return client.Migration(nil), nil
	case "ruling":
		return client.Ruling(nil), nil
	case "set":
