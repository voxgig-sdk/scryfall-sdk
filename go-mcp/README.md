...
MCP server exposing the Scryfall SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `scryfall_list` | `entity`, optional `query` map | First page of records as JSON |
| `scryfall_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `scryfall_list` and
`scryfall_load` tools become available in new sessions.
