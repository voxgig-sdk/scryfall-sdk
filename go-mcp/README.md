...
[MCP](https://modelcontextprotocol.io) server exposing the Scryfall SDK as
two agent tools — `scryfall_list` and `scryfall_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/scryfall-mcp)
make build

# 2. Provide credentials via the environment
export SCRYFALL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user scryfall \
  -- /absolute/path/to/scryfall-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./scryfall-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// scryfall_list: first page of records
{ "entity": "bulk_data" }
{ "entity": "bulk_data", "query": { } }

// scryfall_load: one record by id
{ "entity": "bulk_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/scryfall-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user scryfall \
     -- "$PWD"/dist/*/scryfall-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `scryfall_list` and `scryfall_load` tools now appear
   in new sessions. Ask the agent to *"list bulk_data using scryfall"*
   and it calls `scryfall_list` with `{"entity":"bulk_data"}`.
...
```sh
export SCRYFALL_APIKEY=sk_live_xxx            # API key
export SCRYFALL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `scryfall_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `scryfall_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SCRYFALL_APIKEY` | API key sent with every request. |
| `SCRYFALL_BASE` | Optional override of the API base URL. |
...
```sh
./scryfall-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"scryfall_load","arguments":{"entity":"bulk_data","query":{"id":1}}}}'
```
