...
class ScryfallTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ScryfallTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ScryfallTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ScryfallTestRunner.getenv("SCRYFALL_TEST_LIVE")
        override = ScryfallTestRunner.getenv("SCRYFALL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ScryfallTestRunner.getenv(key)
        ...
        explain = ScryfallTestRunner.getenv("SCRYFALL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SCRYFALL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ScryfallTestRunner._test_control is not None:
            return ScryfallTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ScryfallTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ScryfallTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ScryfallTestRunner.load_test_control()
...
def load_env_local():
    ScryfallTestRunner.load_env_local()


def env_override(m):
    return ScryfallTestRunner.env_override(m)


def entity_data(v):
    return ScryfallTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ScryfallTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ScryfallTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ScryfallTestRunner.load_test_control()


def live_client_options():
    return ScryfallTestRunner.live_client_options()


def live_delay_ms():
    return ScryfallTestRunner.live_delay_ms()
