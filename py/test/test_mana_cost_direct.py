...
class TestManaCostDirect:

    def test_should_direct_list_mana_cost(self):
        setup = _mana_cost_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-mana_cost", "live" if setup["live"] else "unit")
        if _skip:
...
def _mana_cost_direct_setup(mockres):
