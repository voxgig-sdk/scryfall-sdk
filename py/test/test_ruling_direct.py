...
class TestRulingDirect:

    def test_should_direct_list_ruling(self):
        setup = _ruling_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ruling", "live" if setup["live"] else "unit")
        if _skip:
...
def _ruling_direct_setup(mockres):
