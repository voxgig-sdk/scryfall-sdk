...
class TestCardListEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.CardList(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.CardList(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SCRYFALL_TEST_CARD_LIST_ENTID JSON to run live")
        client = setup["client"]
        ...
        card_list_ref01_data = helpers.to_map(runner.entity_data(card_list_ref01_ent.create(card_list_ref01_data, None)))
...
def _card_list_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SCRYFALL_TEST_CARD_LIST_ENTID"))
