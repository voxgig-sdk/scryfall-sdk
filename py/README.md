...
The Python SDK for the Scryfall API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.BulkData(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.BulkData(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Scryfall(None).load(
    {"id": "test01"}, None
)
