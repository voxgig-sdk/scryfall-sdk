...
func TestCardSymbolListEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CardSymbolList(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CardSymbolList(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		cardSymbolListRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.card_symbol_list")))
