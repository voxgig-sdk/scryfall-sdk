...
import (
	...
	omni "github.com/voxgig-sdk/scryfall-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ScryfallError); is {
				out := map[string]any{
					"name":    "ScryfallError",
...
func omniCtx(arg any, client *sdk.ScryfallSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ScryfallSDK); is {
			client = live
