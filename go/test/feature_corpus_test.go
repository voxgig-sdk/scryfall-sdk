...
//
// NewScryfallSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ScryfallSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewScryfallSDK(opts)
}
...
func fcCandidates(client *sdk.ScryfallSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ScryfallSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ScryfallSDK, name string) any {
